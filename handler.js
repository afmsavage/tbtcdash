...
module.exports.findTdts = async (event, wallet) => {
  try {
    ...
    const depositLogContract = new ethers.Contract(
      TBTCSystem.networks['1'].address,
      DepositLog.abi,
      ip
    );
    ...
    const targetKeeps = keeps
      .filter((ev) => {
        return (
          ev.args[1].filter((ms) => {
            return ms.toLowerCase() === opAddr;
          }).length > 0
        );
      })
      .map((ev) => {
    ...
    for (let addr of targetKeeps) {
      ...
      const tdt = await depositLogContract.queryFilter(
        depositLogContract.filters.Created(null, addr)
      );
      if (tdt.length < 1) {
        continue;
      }
      ...
      tdtArray.push(
        `keep ${addr} manages TDT ${d.address} (${ethers.utils.formatEther(
          await d.lotSizeTbtc()
        )} tBTC) with state: ${depositState}`
      );
    }
    const response = {
      statusCode: 200,
